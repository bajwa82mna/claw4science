...
(function () {
  ...
  function renderCard(p) {
    ...
    return `
      <div class="project-card" role="listitem" data-category="${p.category}" data-name="${p.name.toLowerCase()}" data-tags="${p.tags.join(' ').toLowerCase()}" data-desc="${p.desc.toLowerCase()}">
        <div class="card-header">
          <div class="project-icon-placeholder" aria-hidden="true">${p.icon}</div>
          <div class="card-title-row">
            <div class="card-name">${p.name} ${badge}</div>
            <div class="card-meta">
              <span class="card-stars" aria-label="${p.stars} stars">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" style="color:#f0c040" aria-hidden="true"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                ${p.stars}
              </span>
              <span class="card-lang">
                <span class="lang-dot" style="background:${getLangColor(p.langClass)}" aria-hidden="true"></span>
                ${p.lang}
              </span>
            </div>
          </div>
